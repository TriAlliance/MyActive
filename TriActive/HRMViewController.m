...
@property (weak, nonatomic) IBOutlet UIButton *storeHeartBeatBtn;
...
- (void) getHeartBPMData:(CBCharacteristic *)characteristic error:(NSError *)error
{
	...
	if( (characteristic.value)  || !error ) {   // 4
        _storeHeartBeatBtn.enabled=TRUE;
		self.heartRate = bpm;
    ...
    if (storHeartBeat==TRUE) {
        [self storeHeartBeatOnHealthKit];
    }
...
#pragma mark
#pragma mark Health Kit Storage
- (IBAction)storeHeartBeat:(id)sender {
    
    if ([_storeHeartBeatBtn isSelected]==FALSE) {
        storHeartBeat=TRUE;
        _storeHeartBeatBtn.selected=TRUE;
        [self storeHeartBeatOnHealthKit];
    }
    else{
        _storeHeartBeatBtn.selected=FALSE;
        storHeartBeat=FALSE;

    }
}
-(void)storeHeartBeatOnHealthKit{
    /*
*9      */
    NSLog(@"Entering into method");
    NSDate *now = [NSDate date];
    double beatsPerMinute = self.heartRate;
    [[HKManager sharedManager]
     storeHeartBeatsAtMinute:beatsPerMinute
     startDate:now endDate:now
     completion:^(NSError *error) {
         if(error) {
             UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"HealthKit" message:error.description delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
             [av show];
             NSLog(@"ERROR MESSAGE %@",error.description);
         } else {
         }
     }];
//    http://fitness.stackexchange.com/questions/12435/is-it-possible-to-measure-calorie-burn-from-heart-rate-alone
//Male:
//    
...
//                                   date:pauseEnd];
//    HKWorkout *hike = [HKWorkout workoutWithActivityType:HKWorkoutActivityTypeYoga

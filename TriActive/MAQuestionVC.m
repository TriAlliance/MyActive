...
@interface MAQuestionVC ()

@end


@implementation MAQuestionVC
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.titleView = [Utility lblTitleNavBar:@"Questions"];
    ...
    [tblVw_Question addPullToRefreshWithActionHandler:^{
        [weakSelf insertRowAtTop];
    }];
    
    // setup infinite scrolling
    [tblVw_Question addInfiniteScrollingWithActionHandler:^{
        [weakSelf insertRowAtBottom];
    }];
    //[tblVw_Question registerNib:[UINib nibWithNibName:@"MAImageCell" bundle:nil]forCellReuseIdentifier:@"imageCell"];
        ...
        if ([[[arrQuestionData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"image"] && [[[arrQuestionData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"dashboard"])
        {
            ...
            if(cellImageDash ==nil)
            {
                ...
                STTweetLabel *tweetLabel1 = [[STTweetLabel alloc]
                                             initWithFrame:CGRectMake(9.0, 240.0, 299.0, 50.0)];
...
-(IBAction)openUserProfile:(id)sender event:(id)event
{
    ...
    NSIndexPath *indexPath = [tblVw_Question indexPathForRowAtPoint: currentTouchPosition];

...
- (void)viewDidLoad
{
    ...
    arrHomeData  = [[NSMutableArray alloc]init];
    // Do any additional setup after loading the view.
    self.navigationItem.titleView = [Utility lblTitleNavBar:@"Home"];
    ...
    [tbl_Post addPullToRefreshWithActionHandler:^{
        [weakSelf insertRowAtTop];
    }];
    
    // setup infinite scrolling
    [tbl_Post addInfiniteScrollingWithActionHandler:^{
        [weakSelf insertRowAtBottom];
    }];
    
  //[tbl_Post registerNib:[UINib nibWithNibName:@"MAImageCell" bundle:nil]forCellReuseIdentifier:@"imageCell"];
...
-(void)callHomeAPI
{
    ...
    [API userHomeWithInfo:[userInfo mutableCopy] completionHandler:^(NSDictionary *responseDict) {
        ...
        if([[responseDict valueForKey:@"result"] intValue] == 1)
        {
            ...
            for (int i = 0; i < [[responseDict valueForKey:@"data"] count];i++) {
                [arrHomeData  addObject:[[arrPagData objectAtIndex:0] objectAtIndex:i]];
            }

           NSLog(@"arrHomeData --%@",arrHomeData );
            NSLog(@"arrHomeData  count--%lu",(unsigned long)[arrHomeData  count]);
            [tbl_Post reloadData];
        }
        else
        {
//            [ApplicationDelegate showAlert:@"You have no Post on home"];
        ...
        [tbl_Post.pullToRefreshView stopAnimating];
        [tbl_Post.infiniteScrollingView stopAnimating];
...
{
    if([arrHomeData count]){
      if (([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"image"] || [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"video"] || (![[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_image"] isEqualToString:@""])) && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"post"]) {
        return 514.0;
      }
     else if ([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"image"] && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"dashboard"]){
        return 367.0;
        }
     else
     {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([arrHomeData   count]){
    if ([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"image"] && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"dashboard"])
    {
       ...
       UIImageView *imgVw_Post;
        ...
        STTweetLabel *tweetLabel1 = [[STTweetLabel alloc]
                                     initWithFrame:CGRectMake(9.0, 230.0, 299.0, 50.0)];
        
        tweetLabel1.tag=99999;
        tweetLabel1.font = [UIFont fontWithName:@"HelveticaNeue" size:14];
        [tweetLabel1 setText:[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"share_text"]];
        ...
        if(imgVw_Uploaded){
            [imgVw_Uploaded sd_setImageWithURL:[NSURL URLWithString:[[arrHomeData  objectAtIndex:indexPath.row]valueForKey:@"user_profile_image"]] placeholderImage:HomePlaceholder options:SDWebImageRetryFailed];
        }
       imgVw_Post=(UIImageView *)[cellImageDash.contentView viewWithTag:211];
        if(imgVw_Post){
            [imgVw_Post sd_setImageWithURL:[NSURL URLWithString:[[arrHomeData  objectAtIndex:indexPath.row]valueForKey:@"post_image"]] placeholderImage:HomePlaceholder options:SDWebImageRetryFailed];
        }
        lbl_Caption = (UILabel *)[cellImageDash.contentView viewWithTag:207];
       if(lbl_Caption){}
        lbl_like = (UILabel *)[cellImageDash.contentView viewWithTag:201];
        if(lbl_like){
          lbl_like.text=[NSString stringWithFormat:@"%@  Likes",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"]];
        }
        ...
        if(lbl_backSlap){
           lbl_backSlap.text=[NSString stringWithFormat:@"%@  Back Slap",[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"Back Slap"]];
        }
        ...
        if(lbl_bumSlap){
            lbl_bumSlap.text=[NSString stringWithFormat:@"%@  Bum Slap",[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"Bump Slap"]];
        }
        ...
        if(lbl_Comments){
            lbl_Comments.text=[NSString stringWithFormat:@"%@  Comments",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"comments"]];
        }
        ...
        if(lbl_name){
            lbl_name.text=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"user_first_name"];
        }
        lbl_time = (UILabel *)[cellImageDash.contentView viewWithTag:209];
        if(lbl_time){
            lbl_time.text=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"date"];
        }
        
      return cellImageDash;
    }
     else if (([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"image"] && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"post"]) || ((![[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_image"] isEqualToString:@""]) && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"text"])){
        ...
        UIImageView *imgVw_Post;
         ...
         [tweetLabel3 setText:[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"message"]];
        ...
        imgVw_Activity=(UIImageView *)[cellImage.contentView viewWithTag:210];
            ...
            if(imgVw_Activity){
              if ([[imgActivity valueForKey:@"txt"] containsObject: [[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"activity_name"]]) {
                 
                  NSUInteger index = [[imgActivity valueForKey:@"txt"] indexOfObject:[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"activity_name"]];
                  if (index != NSNotFound) {
        ...
        if(imgVw_Post){
            if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_image"] isEqualToString:@""]){
                 [imgVw_Post sd_setImageWithURL:[NSURL URLWithString:[[arrHomeData  objectAtIndex:indexPath.row]valueForKey:@"post_image"]] placeholderImage:HomePlaceholder options:SDWebImageRetryFailed];
            }
            else{[imgVw_Post sd_setImageWithURL:[NSURL URLWithString:[[arrHomeData  objectAtIndex:indexPath.row]valueForKey:@"strava_image"]] placeholderImage:HomePlaceholder options:SDWebImageRetryFailed];}
         }
        
        lbl_like = (UILabel *)[cellImage.contentView viewWithTag:201];
        if(lbl_like){
            cellImage.lbl_like.text=[NSString stringWithFormat:@"%@  Likes",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"]];
       }
        ...
        if (btnLike) {
         ...
         else
         {
            [btn_strava setHidden:NO];
             ...
             if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_distance"]isEqualToString:@""]){
                 [lbl_dist_val setHidden:YES];
                 [lbl_dist setHidden:YES];
                 ...
                 if(lbl_dist_val){
                     ...
                     lbl_dist_val.text=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_distance"];
                 }
             }
             
             lbl_duratn = (UILabel *)[cellImage.contentView viewWithTag:1003];
             lbl_duratn_val= (UILabel *)[cellImage.contentView viewWithTag:1004];
             if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_duration"] isEqualToString:@""]){
                 [lbl_duratn setHidden:YES];
                 [lbl_duratn_val setHidden:YES];
                ...
                if(lbl_duratn_val){
                    [lbl_duratn_val setHidden:NO];
                     lbl_duratn_val.text=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_duration"];
                 }
             }
             lbl_calorie = (UILabel *)[cellImage.contentView viewWithTag:1005];
             lbl_calorie_val = (UILabel *)[cellImage.contentView viewWithTag:1006];
             if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_calories"] isEqualToString:@""]){
                 [lbl_calorie setHidden:YES];
                 [lbl_calorie_val setHidden:YES];
              }
             else{
                     ...
                     lbl_calorie_val.text=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_calories"];
    ...
    else if ([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"type"] isEqualToString:@"video"] && [[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"post"])
    {
        ...
        [tweetLabel4 setText:[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"message"]];
        ...
        if(imgVwThumb){
            [imgVwThumb sd_setImageWithURL:[NSURL URLWithString:[[arrHomeData  objectAtIndex:indexPath.row]valueForKey:@"thumbnails"]] placeholderImage:HomePlaceholder options:SDWebImageRetryFailed];
        }
    ...
    else
    {
       ...
       if(cellStatus ==nil)
        ...
        [tweetLabel5 setText:[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"message"]];
        ...
        if(lbl_like){
            if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"] isEqualToString:@"0"]||[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"] isEqualToString:@"1"]){
            lbl_like.text=[NSString stringWithFormat:@"%@  Like",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"]];
            }
            else{
                 lbl_like.text=[NSString stringWithFormat:@"%@  Like",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"]];
        ...
        if(lbl_name){
            lbl_name.text=[NSString stringWithFormat:@"%@ ",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"user_first_name"]];
        }
        lbl_time = (UILabel *)[cellStatus.contentView viewWithTag:209];
        if(lbl_time){
            lbl_time.text=[NSString stringWithFormat:@"%@ ",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"date"]];
        }
        return cellStatus;
    }
...
{
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:tbl_Post];
    NSIndexPath *indexPath = [tbl_Post indexPathForRowAtPoint:buttonPosition];
    MALog(@"like-indexPath--%ld",(long)indexPath.row);
    NSDictionary* userInfo = @{@"user_id":[UserDefaults valueForKey:@"user_id"],
                               @"post_id":[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"id"],
                               @"type":@"Like"
                               };
    [ApplicationDelegate show_LoadingIndicator];
    [API HomeLikeWithInfo:[userInfo mutableCopy] completionHandler:^(NSDictionary *responseDict) {
        ...
        if([[responseDict valueForKey:@"result"] intValue] == 1)
        {
            UITableViewCell *cell = [tbl_Post cellForRowAtIndexPath:indexPath];
           
            UILabel *lbl_like = (UILabel *)[cell viewWithTag:201];
            NSString *like=[NSString stringWithFormat:@"%d",[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"] intValue] + 1];
            if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"] isEqualToString:@"0"]||[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Like"] isEqualToString:@"1"]){
...
-(void)btnPressed_like_list:(id)sender
{
    ...
    objList.post_id=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"id"],
...
-(void)btnPressed_backSlap:(id)sender
{
    ...
    [API HomeLikeWithInfo:[userInfo mutableCopy] completionHandler:^(NSDictionary *responseDict) {
        ...
        if([[responseDict valueForKey:@"result"] intValue] == 1)
        {
            ...
            NSString *backSlap=[NSString stringWithFormat:@"%d",[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Back Slap"] intValue] + 1];
            if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Back Slap"] isEqualToString:@"0"]||[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Back Slap"] isEqualToString:@"1"]){
...
-(void)btnPressed_bumSlap:(id)sender
{
    ...
    [API HomeLikeWithInfo:[userInfo mutableCopy] completionHandler:^(NSDictionary *responseDict) {
        ...
        if([[responseDict valueForKey:@"result"] intValue] == 1)
        {
            ...
            NSString *bumSlap=[NSString stringWithFormat:@"%d",[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Bump Slap"] intValue] + 1];
            if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Bump Slap"] isEqualToString:@"0"]||[[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"Bump Slap"] isEqualToString:@"1"]){
...
-(void)btnPressed_Comments:(id)sender
{
    ...
    objPComments.post_id=[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"id"];
    objPComments.type=@"";
...
-(void)btnPressed_More:(id)sender
{
    ...
    if([[[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_type"] isEqualToString:@"dashboard"]){
        _postMessage=[NSString stringWithFormat:@"%@ : %@",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"message"],[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"share_text"]];
    }
       else{
          _postMessage= [[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"message"];
       }
    _imageData = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString: [[arrHomeData  objectAtIndex:indexPath.row] valueForKey:@"post_image"]]];
//    MALog(@"%@",_imageData);
    ...
    if([[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"user_id"] isEqualToString:[NSString stringWithFormat:@"%@",[UserDefaults valueForKey:@"user_id"]]]){
        _postId=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"id"];
...
-(void)btnPressed_Play:(id)sender
{
    ...
    new.url=[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"video"];
    [self.navigationController pushViewController:new animated:YES];
}

/********ELIZA********/
#pragma mark
#pragma "Open User Profile"
#pragma marks
-(IBAction)openUserProfile:(id)sender event:(id)event
{
    NSSet *touches = [event allTouches];
    UITouch *touch = [touches anyObject];
    CGPoint currentTouchPosition = [touch locationInView:tbl_Post];
    NSIndexPath *indexPath = [tbl_Post indexPathForRowAtPoint: currentTouchPosition];
    NSLog(@"%ld",(long)indexPath.row);
    
    NSString *str = [NSString stringWithFormat:@"%@",[UserDefaults valueForKey:@"user_id"]];
    NSString *tmp_str_id =[NSString stringWithFormat:@"%@",[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"user_id"]];
    ...
    if([tmp_str_id isEqualToString:str]){
        ...
        obj_OtherUserhome.other_user_ID =[[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"user_id"];
...
-(void)btnPressed_strava:(id)sender{
    ...
    strava.activityID=[NSString stringWithFormat:@"%@", [[arrHomeData objectAtIndex:indexPath.row] valueForKey:@"strava_id"]];
...
-(void)btnPressed_readMore:(id)sender{
   ...
   readMore.Arr_readMore=[arrHomeData objectAtIndex:indexPath.row] ;
